"""Auxiliary decoder heads for UPerNet-style models using GroupNorm."""
...
class AuxiliaryFCNHead(nn.Module):
    """Auxiliary FCN head using GroupNorm instead of BatchNorm."""
    ...
    def __init__(
        ...
        for _ in range(num_convs):
            ...
            convs.append(nn.GroupNorm(num_groups, channels))
    ...
    def forward(self, features: List[torch.Tensor], output_size=None) -> torch.Tensor:
        ...
        if output_size is not None:
            x = nn.functional.interpolate(
                x, size=output_size, mode='bilinear', align_corners=self.align_corners
            )
        return x